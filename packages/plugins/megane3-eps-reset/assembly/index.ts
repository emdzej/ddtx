/**
 * Megane III / Scenic III / Zoe / Fluence EPS reset.
 *
 * Port of `plugins/megane3_eps_reset.py`. Reads the module's state, reports it, then performs
 ...
 *
 * Two sessions, $C0 then $FA, before blanking the dongle.
 *
...
function next(result: string): i32 {
  switch (step) {
    case 0:
      return session('SDS - Start Diagnostic Session $C0');
    case 1:
      return session('SDS - Start Diagnostic Session $FA');
    case 2:
      return read('DataRead.DID - Dongle state');
    case 3:
      return log('Module state read.');
    case 4:
      return write('SRBLID - Dongle blanking', '{}');
    case 5:
